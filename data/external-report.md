...
Сборка 2026-08-31. Файл генерируется `tools/import_external.py`, правки руками бессмысленны.

## Источники

| Источник | Записей | Прошло проверку |
|---|---|---|
| `mobile-whitelist-cidr` | 30228 | 288 |
| `mobile-whitelist-domains` | 910 | — домены в сборку не идут |
| `operator-whitelist-cidr` | 88 | 20 |

После схлопывания в анонсируемые префиксы принято сетей: **192** (потолок 1600).

## Почему отсеяно

| Причина | Сетей |
|---|---|
| не российская сеть | 22604 |
| нет в таблице IP→ASN или сеть не анонсируется | 2330 |
| ASN не обслуживает ни один сервис каталога | 1373 |
| сеть лежит в двух разных ASN | 1348 |
| адресное пространство оператора связи | 1306 |
| глобальный CDN или облако | 668 |
| уже покрыто снапшотом | 379 |

Зарубежные сети по странам: US — 5070, GB — 1748, TR — 1726, DE — 1524, FR — 1167, IT — 970, NL — 920, CH — 809, SE — 680, AT — 559, ES — 556, PL — 459. Именно ради этих записей и написан фильтр: попади они в direct, часть трафика ушла бы мимо VPN с домашнего адреса.

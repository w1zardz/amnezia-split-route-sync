...
Сборка 2026-09-12. Файл генерируется `tools/import_external.py`, правки руками бессмысленны.

## Источники

| Источник | Уникальных IP/CIDR | Принято IP/CIDR | Домены | Новые поддомены | Корни |
|---|---:|---:|---:|---:|---:|
| `mobile-whitelist-cidr` | 30228 | 266 | 0 | 0 | 0 |
...
| `lib4u-amnezia` | 1532 | 262 | 1954 | 106 | 1041 |
| `v2fly-ru` | 0 | 0 | 1741 | 9 | 996 |
| `jirnobruh-amnezia` | 768 | 142 | 925 | 176 | 305 |
| `kyoresuas-ru-bypass` | 8799 | 725 | 0 | 0 | 0 |
| `kyoresuas-essential` | 311 | 70 | 0 | 0 | 0 |
| `pincetgore-ip-list` | 1416 | 223 | 0 | 0 | 0 |
| `kozlov-amnezia` | 0 | 0 | 858 | 7 | 19 |
| `egkodin-ru-sites` | 12843 | 1298 | 1105 | 638 | 88 |
| `supertico-domains` | 0 | 0 | 2 | 0 | 1 |
| `pvd-dog-no-vpn` | 1029 | 288 | 0 | 0 | 0 |
| `opencck-iplist` | 1449 | 251 | 0 | 0 | 0 |
| `rockblack-ru` | 334 | 68 | 0 | 0 | 0 |

После объединения внутри проверенных диапазонов IP→ASN принято сетей: **1000** (потолок 1000).

⚠️ Потолок срезал **459** сетей, прошедших проверку. Подними `--limit`, если Amnezia переваривает список, или сузь источники.

## Почему отсеяно

| Причина | Сетей |
|---|---|
| не российская сеть | 24383 |
| ASN не обслуживает ни один сервис каталога | 13339 |
| нет в таблице IP→ASN или сеть не анонсируется | 5896 |
| сеть пересекает разные ASN | 4298 |
| уже покрыто снапшотом | 2927 |
| адресное пространство оператора связи | 2860 |
| глобальный CDN или облако | 1456 |
| не публичная сеть | 19 |
| шире допустимой /12 | 8 |

Зарубежные сети по странам: US — 5232, GB — 1884, TR — 1752, DE — 1628, FR — 1196, NL — 1016, IT — 990, CH — 943, SE — 714, ES — 608, AT — 569, PL — 466. Именно ради этих записей и написан фильтр: попади они в direct, часть трафика ушла бы мимо VPN с домашнего адреса.
...
| не резолвится (NXDOMAIN или нет A-записей) | 247 |
| глобальный CDN или облако | 157 |
| нет ответа DNS | 12 |
| не публичный адрес | 7 |
| адрес вне таблицы IP→ASN | 3 |

## Домены вне каталога — 926

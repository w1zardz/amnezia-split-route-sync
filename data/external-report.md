...
Сборка 2026-09-09. Файл генерируется `tools/import_external.py`, правки руками бессмысленны.

## Источники

| Источник | Уникальных IP/CIDR | Принято IP/CIDR | Домены | Новые поддомены |
|---|---:|---:|---:|---:|
| `mobile-whitelist-cidr` | 30228 | 267 | 0 | 0 |
...
| `lib4u-amnezia` | 1477 | 259 | 1954 | 110 |

После объединения внутри проверенных диапазонов IP→ASN принято сетей: **227** (потолок 1600).

## Почему отсеяно

| Причина | Сетей |
|---|---|
| не российская сеть | 22772 |
| нет в таблице IP→ASN или сеть не анонсируется | 2678 |
| ASN не обслуживает ни один сервис каталога | 1520 |
| адресное пространство оператора связи | 1379 |
| сеть пересекает разные ASN | 1193 |
| уже покрыто снапшотом | 911 |
| глобальный CDN или облако | 796 |

Зарубежные сети по странам: US — 5057, GB — 1745, TR — 1718, DE — 1525, FR — 1164, IT — 968, NL — 929, CH — 917, SE — 684, ES — 561, AT — 557, PL — 452. Именно ради этих записей и написан фильтр: попади они в direct, часть трафика ушла бы мимо VPN с домашнего адреса.

...
🍏 **Mac в один клик:** скачайте **install-macos.zip** из Assets, распакуйте и
щёлкните по `install-macos.command` правой кнопкой → «Открыть» → «Открыть».
Установщик всё поставит сам и будет обновлять список каждые 6 часов.
